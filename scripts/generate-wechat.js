...
const lines = [
  ...
  `${cleanText(weather.label)}：${cleanText(weather.condition)}，${weather.temperatureC}°C，体感${weather.feelsLikeC}°C，湿度${cleanText(weather.humidity)}，${cleanText(weather.wind)}。`,
...
issue.sections.forEach((section, index) => {
  lines.push(`${sectionNumbers[index]}、${cleanText(section.title).replace(/\s*10\s*条$/, "")}`);
  section.items.forEach((item, itemIndex) => {
    lines.push(`${itemIndex + 1}. ${cleanText(item.summary)}`);
  });
  lines.push("");
});
